...
# Rebuild index
{
    ...
    for section in $dirs; do
        ...
        [ -z "$files" ] && continue
        # Capitalize first letter for header (portable; GNU sed's \U is not in BSD sed)
        header="$(echo "$section" | awk '{print toupper(substr($0,1,1)) substr($0,2)}')"
        printf '\n## %s\n' "$header"
        echo "$files" | emit_files
    done

    # Standalone files: no slash AND name is not also a section directory
    # (otherwise a foo.md sitting next to foo/ would be listed twice)
    standalone=$(echo "$disk" | grep -v '/' || true)
    if [ -n "$standalone" ] && [ -n "$dirs" ]; then
